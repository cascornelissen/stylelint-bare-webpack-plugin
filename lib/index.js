...
const StylelintError = require('./error');
const StylelintWarning = require('./warning');
    ...
    report(compilation) {
        if ( this.options.emitErrors ) {
            if ( this.errors.length ) {
                compilation.errors.push(new StylelintError(this.options.formatter(this.errors)));
            }

            if ( this.warnings.length ) {
                compilation.warnings.push(new StylelintWarning(this.options.formatter(this.warnings)));
            }
        } else {
            if ( this.warnings.length || this.errors.length ) {
                compilation.warnings.push(new StylelintError(this.options.formatter(this.errors.concat(this.warnings))));
            }
        }
